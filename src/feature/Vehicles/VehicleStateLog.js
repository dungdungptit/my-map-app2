...
import { getVehicleStateLog } from '../../store/reducers/vehicleSlice';
import { DataGrid } from '@mui/x-data-grid';

const VehicleStateLog = ({ vehicleId }) => {
    const [vehicleStateLog, setVehicleStateLog] = useState([]);

    useEffect(() => {
        getVehicleStateLog(vehicleId).then((data) => {
            setVehicleStateLog(data);
            console.log(data);
        });
    }, []);
    ...
    return (
        <Box sx={{ width: '100%', height: 400, mt: 4 }}>
            ...
            <Box sx={{ width: '100%', height: 400, mb: 2 }}>
                <DataGrid
                    rows={vehicleStateLog} columns={columns}
                    disableColumnMenu
